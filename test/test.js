...
describe("Histogram class (private, responsible for auto thresholding)", function () {
	...
	describe("#getStats", function () {
		...
		it("produces expected stats object for entire histogram", function () {
			...
			assert.deepEqual(toFixedDeep(histogram.getStats(), 4), toFixedDeep(expectedValue, 4))
		})
		...
		it("produces expected stats object for histogram segment", function () {
			...
			assert.deepEqual(toFixedDeep(histogram.getStats(60, 180), 4), toFixedDeep(expectedValue, 4))
		})
...
describe("Potrace class", function () {
	...
	describe("behaves predictably in edge cases", function () {
		...
		before(function () {
			bwBlackThreshold0 = fs.readFileSync("./reference-copies/potrace-bw-black-threshold-0.svg", {
				encoding: "utf8"
			})
			bwBlackThreshold255 = fs.readFileSync("./reference-copies/potrace-bw-black-threshold-255.svg", {
				encoding: "utf8"
			})
			bwWhiteThreshold0 = fs.readFileSync("./reference-copies/potrace-bw-white-threshold-0.svg", {
				encoding: "utf8"
			})
			bwWhiteThreshold255 = fs.readFileSync("./reference-copies/potrace-bw-white-threshold-255.svg", {
				encoding: "utf8"
			})

			wbWhiteThreshold0 = fs.readFileSync("./reference-copies/potrace-wb-white-threshold-0.svg", {
				encoding: "utf8"
			})
			wbWhiteThreshold255 = fs.readFileSync("./reference-copies/potrace-wb-white-threshold-255.svg", {
				encoding: "utf8"
			})
			wbBlackThreshold0 = fs.readFileSync("./reference-copies/potrace-wb-black-threshold-0.svg", {
				encoding: "utf8"
			})
			wbBlackThreshold255 = fs.readFileSync("./reference-copies/potrace-wb-black-threshold-255.svg", {
				encoding: "utf8"
			})
		})
...
describe("Posterizer class", function () {
	...
	describe("#_getRanges", function () {
		...
		it("correctly handles predefined array of color stops", function () {
			...
			getColorStops().should.be.deepEqual(
				[16, 26, 50, 128, 211, 212],
				"Duplicated items should be present only once"
			)
	...
	describe("#getSVG", function () {
		...
		it("produces expected results with different thresholds", function () {
			...
			instanceYao.setParameters({ threshold: 128 })
			expected = fs.readFileSync("./reference-copies/posterized-yao-black-threshold-128.svg", {
				encoding: "utf8"
			})
			assert.equal(instanceYao.getSVG(), expected, "Image with threshold 128 does not match with reference copy")
			...
			instanceYao.setParameters({ threshold: 170 })
			expected = fs.readFileSync("./reference-copies/posterized-yao-black-threshold-170.svg", {
				encoding: "utf8"
			})
			assert.equal(instanceYao.getSVG(), expected, "Image with threshold 170 does not match with reference copy")
		})
		...
		it("produces expected white on black image with threshold 170", function (done) {
			...
			instance.loadImage("sources/clouds.jpg", function (err) {
				...
				var expected = fs.readFileSync("./reference-copies/posterized-clouds-white-40.svg", {
					encoding: "utf8"
				})
				var actual = instance.getSVG()
	...
	describe("#posterize", function () {
		...
		it("works with three arguments", function (done) {
			lib.posterize(jimpInstance, { threshold: 170 }, function (err, svgContents) {
				...
				var expected = fs.readFileSync("./reference-copies/posterized-bw-threshold-170.svg", {
					encoding: "utf8"
				})
